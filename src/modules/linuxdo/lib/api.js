...
export const LINUXDO_BASE = '/linuxdo';

export function appPath(to) {
  if (to == null) return LINUXDO_BASE;
  if (typeof to === 'number') return to;
  if (typeof to !== 'string') {
    if (to && typeof to.pathname === 'string') return { ...to, pathname: appPath(to.pathname) };
    return to;
  }
  if (/^(https?:|mailto:|tauri:)/i.test(to) || to.startsWith('#') || to.startsWith('?')) return to;
  if (to.startsWith(LINUXDO_BASE) || to.startsWith('/board')) return to;
  if (to.startsWith('/')) {
    const match = to.match(/^([^?#]*)(.*)$/);
    const pathname = match[1];
    const rest = match[2];
    return (pathname === '/' ? LINUXDO_BASE : LINUXDO_BASE + pathname) + rest;
  }
  return to;
}

export function topicPath(id, postNumber) {
  return appPath(`/topic/${id}${postNumber ? `/${postNumber}` : ''}`);
}
...
export function internalPath(href) {
  ...
  if (user) return appPath(`/user/${encodeURIComponent(decodeURIComponent(user[1]))}`);
  return null;
}
