...
import { Bell, Bookmark, ChevronRight, Compass, FilePenLine, Flame, History, LayoutGrid, LogOut, Menu, MessageCircle, MessagesSquare, Plus, Search, Settings, Sparkles, Trophy, Users, X, ArrowLeft } from 'lucide-react';
...
export default function Shell() {
  ...
  return <div className={`app-shell ${settings.compact ? 'compact' : ''}`}>
    ...
    <aside className={`sidebar ${open ? 'open' : ''}`}>
      ...
      <Link className="brand" to="/linuxdo"><img src="/fluxdo.png" alt="" /><span>Flux<span className="brand-accent">DO</span><small>与好奇心同行</small></span></Link>
      <nav aria-label="主导航" className="sidebar-nav">
        {navigation.map(group => <div className="nav-group" key={group.label}><p>{group.label}</p>{group.items.map(([to, title, Icon]) => <NavLink key={to} to={`/linuxdo${to}`} end={to === '/'} className={({ isActive }) => `nav-item ${isActive ? 'active' : ''}`}><Icon size={19} strokeWidth={1.75} /><span>{title}</span>{to === '/notifications' && unread > 0 && <b className="badge-count">{unread > 99 ? '99+' : unread}</b>}</NavLink>)}</div>)}
        {site.data?.categories?.length > 0 && <div className="nav-group category-nav"><p>常逛的分类<Link to="/linuxdo/categories" aria-label="查看全部分类"><ChevronRight size={16} /></Link></p>{site.data.categories.filter(category => !category.parent_category_id).slice(0, 6).map(category => <NavLink to={`/linuxdo/category/${category.id}`} key={category.id} className="nav-item"><span className="category-dot" style={{ background: /^[0-9a-f]{6}$/i.test(category.color || '') ? `#${category.color}` : 'var(--accent)' }} /><span>{category.name}</span></NavLink>)}</div>}
      </nav>
      <div className="sidebar-bottom"><NavLink className="nav-item" to="/linuxdo/settings"><Settings size={19} strokeWidth={1.75} /><span>设置</span></NavLink><div className="community-caption"><span className="online-dot" />Linux.do 技术社区</div></div>
    </aside>
    <div className="app-main">
      <header className="topbar">
        ...
        <form className="global-search" onSubmit={event => { event.preventDefault(); if (query.trim()) navigate(`/linuxdo/search?q=${encodeURIComponent(query.trim())}`); }}><Search size={18} /><input ref={search} aria-label="搜索话题、用户和标签" placeholder="搜索话题、用户和标签…" value={query} onChange={event => setQuery(event.target.value)} /><kbd>Ctrl K</kbd></form>
        <div className="topbar-actions"><button className="button primary compose-shortcut" onClick={() => user ? navigate('/linuxdo/compose') : requestLogin()}><Plus size={17} /><span>发布话题</span></button><Link to="/linuxdo/notifications" className="icon-button" aria-label="通知"><Bell size={20} />{unread > 0 && <i className="notification-dot" />}</Link>{user ? <><Link to={`/linuxdo/user/${encodeURIComponent(user.username)}`} className="current-user" title={user.username}><Avatar user={user} size={33} /><span>{user.username}</span></Link><button className="icon-button" title="退出登录" aria-label="退出登录" onClick={() => logout().catch(error => notify(errorText(error)))}><LogOut size={18} /><span className="sr-only">退出登录</span></button></> : <button className="button secondary" onClick={requestLogin}>登录</button>}</div>
      </header>
