...
### ☁️ 跨设备同步空间
- **端到端加密**：知识笔记与四列看板在客户端加密，`server_master` 只保存密文
- **手机互通**：与 Android Alive 使用同一 Vault、设备配对和冲突副本策略
- **权限隔离**：同步空间运行在独立 WebView 中，不获得论坛客户端的 Tauri IPC 权限
- **非破坏迁移**：打开同步空间会按稳定 ID 合并原有卡片与知识笔记，原 localStorage 保持不变

完成迁移后，同步空间是跨设备数据的权威入口；手机修改不会反写到旧 `board-data-v1`。原看板卡片里的内嵌图片不会自动迁移：同步 API 的单条密文上限为 1 MiB，而本地卡片最多可含四张压缩图。文本与知识库字段会正常合并，图片继续保留在原本地看板，可在同步空间中作为加密附件重新添加。
