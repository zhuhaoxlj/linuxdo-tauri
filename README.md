...
- **局域网直连**：订阅中继拿到手机的局域网地址后，主动握手验证（HMAC 挑战应答）才认定为可用通道，
  侧边栏底部实时显示直连状态与 RTT。P1 只做发现 + 验证，传输层（大文件 / 实时音频）后续接入
- **短信验证码**：手机解析出验证码后优先走局域网（长轮询推送，不出公网），
  不在同一网络时经中继兜底；收到后屏幕正中弹出卡片，可一键复制
- **电脑音频推到手机**：侧边栏一键把系统输出（PipeWire/PulseAudio monitor）以裸 PCM 经 UDP 推给
  手机播放，手机连蓝牙耳机时自动走 A2DP；**开启时自动静音电脑本地输出**（否则电脑音箱与手机
  差几百毫秒同时出声，听起来像回声），停止时恢复；手机静默 3 秒后自动停止播放
